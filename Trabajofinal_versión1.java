...
public class Trabajofinal {
    ...
    public static void main(String[] args) {
        ...
        String tieneMedidor = "no";
        String tipoUso = "comercial";
    ...
    private static double obtenerCostoDomestico(int mCubico) {
        double costo=0.0;
        for(int i=1; i<=mCubico; i++){
            if (i>=1 && i<=29){
                costo = costo + 0.42;
            }else if (i>29){
                costo = costo + 1.56;
    ...
    private static double obtenerCostoComercial(int mCubico) {
        double costo=0.0;
        for(int i=1; i<=mCubico; i++){
            if (i>=1 && i<=99){
                costo = costo + 1.56;
    ...
    private static double obtenerCostoIndustrial(int mCubico) {
        double costo=0.0;
        for(int i=1; i<=mCubico; i++){
            if (i>=1 && i<=399){
                costo = costo + 2.36;
    ...
    private static double obtenerCostoEstatal(int mCubico) {
        double costo=0.0;
        for(int i=1; i<=mCubico; i++){
            if (i>=1 && i<=1999){
                costo = costo + 1.56;
    ...
    private static double obtenerCosSegunUso(int mCubico,String tipoUso) {
        double costo = switch (tipoUso) {
            case "doméstico" -> obtenerCostoDomestico(mCubico);
            case "comercial" -> obtenerCostoComercial(mCubico);
            case "industrial" -> obtenerCostoIndustrial(mCubico);
            case "estatal" -> obtenerCostoEstatal(mCubico);
            default -> 0.0;
        };
        return costo;
    }
